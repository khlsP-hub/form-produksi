...
const getShiftValue = (shift, beratStr) => {
  ...
  const reject=shiftTotalReject(shift), output=parseNum(shift.outputRaw??shift.output), berat=parseNum(beratStr);
  if (output>0&&berat>0) { const outKg=(output*berat)/1000; return {value:(reject/outKg)*100,isPct:true}; }
...
function buildMesinData(docs, noMesin) {
  ...
  return asistens.map((nama, i) => {
    ...
    return {nama,color:ASISTEN_COLORS[i%3],bg:ASISTEN_BG[i%3],points,totalKg,avgPct,over3,trend,avgPerShift,count:points.length};
  });
}
...
// ─── REDESIGNED: SummarySection (menggantikan inline summary di ReportScreen) ──
function SummarySection({ stats, activeReport }) {
  return (
    <View style={sc.summaryWrapper}>

      {/* Kartu utama: form & reject */}
      ...
      <View style={sc.row}>
        ...
        />
        <SummaryStatCard
          num={stats.totalReject.toFixed(1)}
          label="Total Reject"
          unit=" KG"
          accentColor="#A32D2D"
          bgColor="#FCEBEB"
      ...
      <TouchableOpacity style={apc.header} onPress={()=>setOpen(o=>!o)} activeOpacity={0.85}>
        <View style={apc.left}>
          <View style={apc.shiftRow}>
            <View style={[apc.shiftBadge,{backgroundColor:ASISTEN_COLORS[point.shiftNum-1]||'#1565C0'}]}>
...
function AsistenCompactCard({ data }) {
  const [open,setOpen]=useState(false);
  return(
    <View style={[komp.compCard,{borderTopColor:data.color,borderLeftColor:ASISTEN_BORDER[ASISTEN_COLORS.indexOf(data.color)]||data.color}]}>
        ...
        <View style={pick.sheet}>
          ...
          {mode==='multi'&&selectedMany.length>0&&(
            <View style={pick.chipsRow}>
              {selectedMany.map((nama,i)=>(<View key={nama} style={[pick.chip,{backgroundColor:ASISTEN_BG[i],borderColor:ASISTEN_COLORS[i]}]}><View style={[pick.chipNum,{backgroundColor:ASISTEN_COLORS[i]}]}><Text style={pick.chipNumTxt}>{i+1}</Text></View><Text style={[pick.chipName,{color:ASISTEN_COLORS[i]}]}>{nama}</Text></View>))}
            </View>
          )}
          ...
          <FlatList data={filtered} keyExtractor={item=>item} keyboardShouldPersistTaps="handled"
            renderItem={({item})=>{
              if (mode==='single') {
                ...
                return(<TouchableOpacity style={[pick.item,isSel&&{backgroundColor:ASISTEN_BG[selIdx]},isMaxed&&{opacity:0.4}]}
                  ...
                  <View style={pick.itemLeft}>
                    <View style={[pick.avatar,isSel&&{backgroundColor:ASISTEN_COLORS[selIdx]}]}>
                      <Text style={[pick.avatarTxt,isSel&&pick.avatarTxtActive]}>{isSel?selIdx+1:item.charAt(0).toUpperCase()}</Text>
                    </View>
                    <Text style={[pick.itemName,isSel&&{color:ASISTEN_COLORS[selIdx],fontWeight:'700'}]}>{item}</Text>
                  </View>
                  {isSel?<Ionicons name="checkmark-circle" size={18} color={ASISTEN_COLORS[selIdx]}/>:isMaxed?<Text style={{fontSize:10,color:'#ccc'}}>Maks {maxMany}</Text>:null}
                </TouchableOpacity>);
              }
            }}
...
function BandingkanPanel({ allDocs, initRangeDocs }) {
  ...
  const compData=useMemo(()=>{
    ...
    return selected.map((nama,i)=>{
      ...
      return{nama,color:ASISTEN_COLORS[i],bg:ASISTEN_BG[i],points,totalKg,avgPct,over3,trend,avgPerShift,count:points.length};
    });
        ...
        <View style={ap.chipsRow}>
          {selected.map((nama,i)=>(
            <View key={nama} style={[ap.chip,{backgroundColor:ASISTEN_BG[i],borderColor:ASISTEN_COLORS[i]}]}>
              <View style={[ap.chipNum,{backgroundColor:ASISTEN_COLORS[i]}]}><Text style={ap.chipNumTxt}>{i+1}</Text></View>
              <Text style={[ap.chipName,{color:ASISTEN_COLORS[i]}]} numberOfLines={1}>{nama}</Text>
              <TouchableOpacity onPress={()=>toggle(nama)} hitSlop={{top:8,bottom:8,left:4,right:4}}>
                <Ionicons name="close-circle" size={15} color={ASISTEN_COLORS[i]}/>
              </TouchableOpacity>
            </View>
          ))}
          {selected.length<3&&(
...
function GroupChartBox({ pcts }) {
  ...
  return(
    <View style={ch.box}>
      ...
      <Text style={ch.note}>Persentase: reject (KG) ÷ output (KG) × 100</Text>
    </View>
  );
}
...
function DateGroup({ dateLabel, docs, defaultOpen, activeReport }) {
  ...
  const totalReject=docs.reduce((sum,d)=>sum+[1,2,3].reduce((s2,n)=>s2+shiftTotalReject(d[`shift${n}`]),0),0);
  const groupPcts=[1,2,3].map(n=>{let tot=0,cnt=0;docs.forEach(d=>{const sv=getShiftValue(d[`shift${n}`],d.berat);if(sv){tot+=sv.value;cnt++;}});return cnt>0?tot/cnt:null;});
  ...
  return(
    <View style={dg.wrapper}>
      <TouchableOpacity style={dg.header} onPress={()=>setOpen(o=>!o)} activeOpacity={0.85}>
        <View style={dg.left}><Text style={dg.date}>{dateLabel}</Text><Text style={dg.sub}>{docs.length} form · {totalReject.toFixed(2)} KG reject</Text></View>
      ...
      {open&&<View style={dg.body}><GroupChartBox pcts={groupPcts}/>{docs.map(d=><FormCard key={d.id} doc={d} activeReport={activeReport}/>)}</View>}
    </View>
  );
}
...
export default function ReportScreen() {
  ...
  const fetchData=useCallback(async(selectedMode)=>{
    ...
    try{
      let allDocs=[];
      if(selectedMode==='tahunan'){const year=new Date().getFullYear();const snap=await getDocs(collection(db,'form_produksi'));allDocs=snap.docs.map(d=>({id:d.id,...d.data()})).filter(d=>{const p=(d.tanggal||'').split('/');return p.length===3&&parseInt(p[2])===year;});}
      else{allDocs=await fetchDocsByDates(getDateRange(selectedMode));}
      const map={};
      allDocs.forEach(doc=>{const key=`${doc.tanggal}_${doc.noMesin}_${doc.kodeProduk||doc.namaProduk}`;if(!map[key]){map[key]={...doc};}else{if(isShiftFilled(doc.shift1))map[key].shift1=doc.shift1;if(isShiftFilled(doc.shift2))map[key].shift2=doc.shift2;if(isShiftFilled(doc.shift3))map[key].shift3=doc.shift3;if(doc.berat&&!map[key].berat)map[key].berat=doc.berat;}});
      setDocs(Object.values(map));setLastFetch(new Date());
    }catch(e){console.error(e);}finally{setLoading(false);}
  },[]);

  useEffect(()=>{if(activeReport!==3&&activeReport!==4)fetchData(mode);},[mode,activeReport]);

  const filteredDocs=useMemo(()=>activeReport===2?docs.filter(d=>[1,2,3].some(n=>{const sv=getShiftValue(d[`shift${n}`],d.berat);return sv?.isPct&&sv.value>=3;})):docs,[docs,activeReport]);
  const grouped=useMemo(()=>{
    const map={};
    filteredDocs.forEach(d=>{let key=d.tanggal||'?';if(mode==='tahunan'){const p=key.split('/');key=p.length===3?`${p[1]}/${p[2]}`:key;}if(!map[key])map[key]=[];map[key].push(d);});
    const bulan=['Jan','Feb','Mar','Apr','Mei','Jun','Jul','Agt','Sep','Okt','Nov','Des'];
    return Object.entries(map).sort(([a],[b])=>b.split('/').reverse().join('').localeCompare(a.split('/').reverse().join(''))).map(([k,ds])=>{if(mode==='tahunan'){const[mm,yy]=k.split('/');return{label:`${bulan[parseInt(mm)-1]} ${yy}`,docs:ds};}return{label:k,docs:ds};});
  },[filteredDocs,mode]);

  const stats=useMemo(()=>{
    let totalReject=0,naikCount=0,turunCount=0,fluktCount=0,over3Count=0;
    filteredDocs.forEach(d=>{[1,2,3].forEach(n=>{totalReject+=shiftTotalReject(d[`shift${n}`]);});const sv=[1,2,3].map(n=>getShiftValue(d[`shift${n}`],d.berat));if(sv.some(s=>s?.isPct&&s.value>=3))over3Count++;});
    grouped.forEach(g=>{const pcts=[1,2,3].map(n=>{let tot=0,cnt=0;g.docs.forEach(d=>{const sv=getShiftValue(d[`shift${n}`],d.berat);if(sv){tot+=sv.value;cnt++;}});return cnt>0?tot/cnt:null;});const tr=getOverallTrend(pcts);if(tr?.id==='naik')naikCount++;else if(tr?.id==='turun')turunCount++;else if(tr?.id==='fluktuatif')fluktCount++;});
    return{total:filteredDocs.length,totalReject,naikCount,turunCount,fluktCount,over3Count};
  },[filteredDocs,grouped]);

  return(
    <View style={s.container}>
      {/* Header */}
      <View style={s.header}>
        <View style={s.headerBadge}><Ionicons name="bar-chart" size={12} color="#90CAF9"/><Text style={s.headerBadgeTxt}>LAPORAN ANALISIS</Text></View>
        <Text style={s.headerTitle}>{REPORT_TYPES.find(r=>r.id===activeReport)?.label||'Laporan'}</Text>
        {lastFetch&&activeReport<=2&&<Text style={s.headerSub}>Diperbarui {lastFetch.toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit'})}</Text>}
      </View>

      {/* Tab tipe laporan */}
      <View style={s.reportBar}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.reportBarInner}>
          {REPORT_TYPES.map(rt=>(<TouchableOpacity key={rt.id} style={[s.reportTab,activeReport===rt.id&&s.reportTabActive]} onPress={()=>setActiveReport(rt.id)}><Ionicons name={rt.icon} size={13} color={activeReport===rt.id?'#fff':'#1565C0'}/><Text style={[s.reportTabTxt,activeReport===rt.id&&s.reportTabTxtActive]}>{rt.label}</Text></TouchableOpacity>))}
        </ScrollView>
      </View>

      {/* Content */}
      {(activeReport===3||activeReport===4) ? (
        <PerAsistenScreen allDocs={docs}/>
      ) : (
        <>
          <View style={s.modeBar}>
            {MODES.map(m=>(<TouchableOpacity key={m.key} style={[s.modeBtn,mode===m.key&&s.modeBtnActive]} onPress={()=>setMode(m.key)}><Text style={[s.modeTxt,mode===m.key&&s.modeTxtActive]}>{m.label}</Text></TouchableOpacity>))}
          </View>
          {loading?(
            <View style={s.center}><ActivityIndicator size="large" color="#1565C0"/><Text style={s.loadTxt}>Memuat data...</Text></View>
          ):(
            <ScrollView style={s.scroll} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={loading} onRefresh={()=>fetchData(mode)} colors={['#1565C0']}/>}>
              ...
              <View style={s.legend}>
                ...
                <View style={s.legendRow}>
                  {[{icon:'trending-up',color:'#c62828',lbl:'Reject naik'},{icon:'pulse',color:'#e65100',lbl:'Fluktuatif'},{icon:'trending-down',color:'#2e7d32',lbl:'Reject turun'}].map((l,i)=>(<View key={i} style={s.legendItem}><Ionicons name={l.icon} size={12} color={l.color}/><Text style={s.legendTxt}>{l.lbl}</Text></View>))}
                  <View style={s.legendItem}><View style={s.legendDot}/><Text style={s.legendTxt}>Reject ≥3%</Text></View>
                </View>
              </View>

              {grouped.length===0?(<View style={s.empty}><Ionicons name="checkmark-done-circle-outline" size={54} color="#a5d6a7"/><Text style={s.emptyTxt}>Tidak ada data</Text><Text style={s.emptySub}>{activeReport===2?'Tidak ada reject ≥3% pada periode ini':'Belum ada form yang tersimpan'}</Text></View>):(<View style={s.list}>{grouped.map((g,i)=><DateGroup key={g.label} dateLabel={g.label} docs={g.docs} defaultOpen={i===0} activeReport={activeReport}/>)}</View>)}
              <View style={{height:40}}/>
            </ScrollView>
          )}
        </>
...
const s=StyleSheet.create({
  ...
  modeBar:{flexDirection:'row',backgroundColor:'#fff',paddingHorizontal:12,paddingVertical:8,borderBottomWidth:1,borderBottomColor:'#E8EDF5',gap:6},
  modeBtn:{flex:1,paddingVertical:7,borderRadius:8,alignItems:'center',backgroundColor:'#F0F4FA'},
  modeBtnActive:{backgroundColor:'#1565C0'},modeTxt:{fontSize:11,fontWeight:'700',color:'#888'},modeTxtActive:{color:'#fff'},
  scroll:{flex:1},center:{flex:1,justifyContent:'center',alignItems:'center',padding:40},loadTxt:{color:'#888',marginTop:10,fontSize:13},
  ...
  emptyTxt:{fontSize:16,fontWeight:'700',color:'#90a4ae'},emptySub:{fontSize:12,color:'#bbb',textAlign:'center'},
});

// ─── REDESIGNED: Summary Card Styles ─────────────────────────────────────────
const sc=StyleSheet.create({
  ...
  card:{
    ...
    // subtle left separator accent
    borderLeftWidth:1,
    borderLeftColor:'#E8EDF5',
    borderRightWidth:1,
...
const ap=StyleSheet.create({
  ...
  selectorTxt:{flex:1,fontSize:14,color:'#aaa'},selectorTxtFilled:{color:'#1565C0',fontWeight:'700'},
  hint:{fontSize:11,color:'#f57f17',marginTop:-10,marginBottom:12,fontStyle:'italic'},
  ...
  emptyTxt:{fontSize:15,fontWeight:'700',color:'#90a4ae'},emptySub:{fontSize:12,color:'#bbb',textAlign:'center',lineHeight:18},
  ...
  resultName:{color:'#fff',fontSize:16,fontWeight:'800'},resultRange:{color:'rgba(255,255,255,0.7)',fontSize:11,marginTop:2},
...
const sh=StyleSheet.create({
  ...
  trendTxt:{fontSize:10,fontWeight:'700'},trendTxtSm:{fontSize:9},
...
const dg=StyleSheet.create({
  ...
  left:{flex:1},date:{color:'#fff',fontSize:15,fontWeight:'800'},sub:{color:'rgba(255,255,255,0.7)',fontSize:11,marginTop:2},
  right:{flexDirection:'row',alignItems:'center',gap:8},
  trendBadge:{flexDirection:'row',alignItems:'center',gap:4,paddingHorizontal:8,paddingVertical:4,borderRadius:8},trendTxt:{fontSize:10,fontWeight:'700'},
...
const fc=StyleSheet.create({
  ...
  cardWarn:{borderLeftColor:'#e53935'},header:{flexDirection:'row',justifyContent:'space-between',padding:12,alignItems:'flex-start'},
  left:{flex:1,marginRight:8},badgeRow:{flexDirection:'row',flexWrap:'wrap',gap:5,marginBottom:6},
  badge:{flexDirection:'row',alignItems:'center',gap:3,paddingHorizontal:6,paddingVertical:3,borderRadius:6},badgeTxt:{fontSize:9,fontWeight:'700'},
  mesin:{fontSize:14,fontWeight:'800',color:'#1565C0'},produk:{fontSize:11,color:'#666',marginTop:2},
  right:{alignItems:'flex-end'},totalVal:{fontSize:20,fontWeight:'800',color:'#e53935'},totalUnit:{fontSize:9,color:'#aaa',marginTop:1},
  expandBtn:{flexDirection:'row',alignItems:'center',gap:3,marginTop:8,backgroundColor:'#F0F4FA',paddingHorizontal:8,paddingVertical:4,borderRadius:8},expandBtnActive:{backgroundColor:'#EEF4FF'},
  ...
  pillHigh:{backgroundColor:'#FFEBEE'},pillEmpty:{backgroundColor:'#F5F5F5',opacity:0.7},
  pillLbl:{fontSize:9,fontWeight:'700',color:'#888',marginBottom:2},pillDash:{fontSize:14,color:'#ccc'},
  pillKg:{fontSize:12,fontWeight:'800',color:'#333'},pillPct:{fontSize:10,color:'#777'},pillRed:{color:'#e53935'},
...
const sd=StyleSheet.create({
  ...
  numBadge:{backgroundColor:'#1565C0',width:28,height:28,borderRadius:14,justifyContent:'center',alignItems:'center'},numTxt:{color:'#fff',fontSize:11,fontWeight:'800'},
  meta:{flex:1},karu:{fontSize:11,color:'#888'},karuVal:{fontWeight:'700',color:'#222'},out:{fontSize:10,color:'#aaa',marginTop:1},
  rejectWrap:{alignItems:'flex-end'},rejectKg:{fontSize:13,fontWeight:'800',color:'#333'},rejectLabel:{fontSize:9,color:'#aaa'},
  body:{paddingHorizontal:11,paddingBottom:10,borderTopWidth:1,borderTopColor:'#F0F4F8'},
  ok:{flexDirection:'row',alignItems:'center',gap:6,backgroundColor:'#F1F8E9',padding:8,borderRadius:6,marginTop:8},okTxt:{fontSize:11,color:'#558b2f'},
  problem:{backgroundColor:'#F8FAFC',borderRadius:7,padding:10,marginTop:8,borderLeftWidth:2,borderLeftColor:'#90CAF9'},problemOpen:{borderLeftColor:'#e53935',backgroundColor:'#FFF5F5'},
  problemTop:{flexDirection:'row',alignItems:'center',gap:6,marginBottom:6,flexWrap:'wrap'},
  problemNum:{backgroundColor:'#E3F2FD',paddingHorizontal:7,paddingVertical:2,borderRadius:4},problemNumTxt:{fontSize:10,fontWeight:'800',color:'#1565C0'},
  asisten:{fontSize:11,fontWeight:'700',color:'#333',flex:1},
  pill:{paddingHorizontal:7,paddingVertical:2,borderRadius:5},pillOpen:{backgroundColor:'#FFEBEE'},pillClose:{backgroundColor:'#E8F5E9'},pillTxt:{fontSize:9,fontWeight:'800'},
  field:{flexDirection:'row',gap:5,marginTop:4,alignItems:'flex-start'},fieldLabel:{fontSize:11,fontWeight:'700',color:'#666'},fieldVal:{fontSize:11,color:'#333',flex:1,lineHeight:16},
});
const ch=StyleSheet.create({
  ...
  barCol:{flex:1,alignItems:'center'},valTxt:{fontSize:10,fontWeight:'800',color:'#444',marginBottom:4},valHigh:{color:'#e53935'},
  track:{width:30,backgroundColor:'#EEF4FF',borderRadius:6,justifyContent:'flex-end',overflow:'hidden'},fill:{width:'100%',borderRadius:6},
  lblTxt:{fontSize:10,fontWeight:'700',color:'#666',marginTop:6},note:{fontSize:9,color:'#aaa',textAlign:'center',marginTop:8},
});
const ach=StyleSheet.create({
  ...
  legendItem:{flexDirection:'row',alignItems:'center',gap:4},legendDot:{width:10,height:10,borderRadius:5},legendTxt:{fontSize:10,color:'#666'},
  barsRow:{flexDirection:'row',alignItems:'flex-end',paddingTop:4},
  barWrap:{alignItems:'center',paddingHorizontal:4,minWidth:56},
  valTxt:{fontSize:9,fontWeight:'800',color:'#444',marginBottom:4,textAlign:'center'},valHigh:{color:'#e53935'},
  track:{width:28,backgroundColor:'#EEF4FF',borderRadius:6,justifyContent:'flex-end',overflow:'hidden'},fill:{width:'100%',borderRadius:6},
  dateTxt:{fontSize:8,color:'#888',marginTop:5,textAlign:'center'},
  shiftDot:{marginTop:3,paddingHorizontal:6,paddingVertical:2,borderRadius:8,alignItems:'center'},shiftDotTxt:{fontSize:8,fontWeight:'800',color:'#fff'},
  threshRow:{flexDirection:'row',alignItems:'center',gap:8,marginTop:8},
  threshLine:{flex:1,height:1,backgroundColor:'#ffcdd2'},threshTxt:{fontSize:9,color:'#e53935',fontWeight:'600'},
...
const apc=StyleSheet.create({
  ...
  cardWarn:{borderLeftColor:'#e53935'},cardOpen:{borderLeftColor:'#e53935'},
  ...
  shiftBadge:{paddingHorizontal:10,paddingVertical:3,borderRadius:8},shiftTxt:{fontSize:11,fontWeight:'800',color:'#fff'},
  openBadge:{flexDirection:'row',alignItems:'center',gap:3,backgroundColor:'#FFEBEE',paddingHorizontal:7,paddingVertical:3,borderRadius:6},openTxt:{fontSize:9,color:'#c62828',fontWeight:'800'},
  tanggal:{fontSize:13,fontWeight:'700',color:'#222'},mesin:{fontSize:11,fontWeight:'700',color:'#1565C0',marginTop:2},
  produk:{fontSize:10,color:'#888',marginTop:1},karu:{fontSize:10,color:'#aaa',marginTop:1},
  right:{alignItems:'flex-end'},pct:{fontSize:22,fontWeight:'800',color:'#333'},pctHigh:{color:'#e53935'},
  pctLabel:{fontSize:9,color:'#aaa'},kg:{fontSize:11,color:'#888',marginTop:1},output:{fontSize:9,color:'#aaa',marginTop:2},
  ...
  row:{backgroundColor:'#fff',borderRadius:8,padding:10,marginBottom:6,borderLeftWidth:2,borderLeftColor:'#90CAF9'},rowOpen:{borderLeftColor:'#e53935',backgroundColor:'#FFF5F5'},
  rowTop:{flexDirection:'row',alignItems:'center',gap:6,marginBottom:6,flexWrap:'wrap'},
  rowNum:{backgroundColor:'#E3F2FD',paddingHorizontal:7,paddingVertical:2,borderRadius:4},rowNumTxt:{fontSize:10,fontWeight:'800',color:'#1565C0'},
  pill:{paddingHorizontal:7,paddingVertical:2,borderRadius:5},pillOpen:{backgroundColor:'#FFEBEE'},pillClose:{backgroundColor:'#E8F5E9'},pillTxt:{fontSize:9,fontWeight:'800'},
  rejectTag:{backgroundColor:'#FFF3E0',paddingHorizontal:7,paddingVertical:2,borderRadius:4},rejectTagTxt:{fontSize:9,color:'#e65100',fontWeight:'700'},
  field:{flexDirection:'row',gap:5,marginTop:4,alignItems:'flex-start'},fieldLbl:{fontSize:11,fontWeight:'700',color:'#666'},fieldVal:{fontSize:11,color:'#333',flex:1,lineHeight:16},
});
const komp=StyleSheet.create({
  ...
  chartHeaderRow:{flexDirection:'row',alignItems:'center',gap:7,marginBottom:10},chartCardTitle:{fontSize:13,fontWeight:'800',color:'#1565C0'},
  chartLegend:{flexDirection:'row',flexWrap:'wrap',gap:12,marginBottom:14},
  chartLegendItem:{flexDirection:'row',alignItems:'center',gap:5},chartLegendDot:{width:10,height:10,borderRadius:5},chartLegendTxt:{fontSize:10,color:'#555',fontWeight:'600'},
  chartArea:{flexDirection:'row',justifyContent:'space-around',alignItems:'flex-end'},
  shiftGroup:{flex:1,alignItems:'center',gap:6},shiftBarsWrap:{flexDirection:'row',alignItems:'flex-end',gap:4},
  barWrap:{alignItems:'center'},barValTxt:{fontSize:9,fontWeight:'800',marginBottom:4},
  barTrack:{width:22,backgroundColor:'#EEF4FF',borderRadius:5,justifyContent:'flex-end',overflow:'hidden'},barFill:{width:'100%',borderRadius:5},
  barDot:{width:8,height:8,borderRadius:4,marginTop:3},
  shiftLabelBox:{backgroundColor:'#EEF4FF',paddingHorizontal:10,paddingVertical:4,borderRadius:8},shiftLabelTxt:{fontSize:10,fontWeight:'800',color:'#1565C0'},
  ...
  winnerLbl:{fontSize:10,color:'#888',fontWeight:'600',marginBottom:3},winnerName:{fontSize:13,fontWeight:'800',marginBottom:4},
  winnerPill:{paddingHorizontal:8,paddingVertical:3,borderRadius:8,borderWidth:1},winnerPillTxt:{fontSize:10,fontWeight:'700'},
  ...
  tableHeaderRow:{flexDirection:'row',alignItems:'center',gap:7,padding:14,borderBottomWidth:1,borderBottomColor:'#EEF0F2'},tableCardTitle:{fontSize:13,fontWeight:'800',color:'#1565C0'},
  tableColHeaders:{flexDirection:'row',paddingHorizontal:12,paddingVertical:8,backgroundColor:'#F8FAFC'},
  tableMetricCol:{width:90},tableNameCol:{flex:1,alignItems:'center',borderBottomWidth:2,marginHorizontal:3,paddingBottom:4},tableNameTxt:{fontSize:11,fontWeight:'800'},
  tableRow:{flexDirection:'row',alignItems:'center',paddingHorizontal:12,paddingVertical:10},tableRowAlt:{backgroundColor:'#F8FAFC'},
  tableMetricTxt:{width:90,fontSize:11,color:'#666',fontWeight:'600'},tableValCell:{flex:1,alignItems:'center'},tableValTxt:{fontSize:12,fontWeight:'800'},
  ...
  compAvatar:{width:36,height:36,borderRadius:18,justifyContent:'center',alignItems:'center'},compAvatarTxt:{color:'#fff',fontSize:16,fontWeight:'800'},
  compName:{fontSize:13,fontWeight:'800'},compMeta:{fontSize:10,color:'#888',marginTop:2},
  compWarnTag:{flexDirection:'row',alignItems:'center',gap:3,backgroundColor:'#FFF3E0',paddingHorizontal:7,paddingVertical:3,borderRadius:6},compWarnTxt:{fontSize:9,color:'#e65100',fontWeight:'700'},
  compBody:{paddingHorizontal:10,paddingBottom:10,gap:6},compEmpty:{fontSize:12,color:'#aaa',textAlign:'center',paddingVertical:12},
});
const pick=StyleSheet.create({
  ...
  headerTxt:{color:'#fff',fontSize:16,fontWeight:'700'},headerSub:{color:'rgba(255,255,255,0.7)',fontSize:11,marginTop:2},
  closeBtn:{padding:4},
  ...
  chipNum:{width:16,height:16,borderRadius:8,justifyContent:'center',alignItems:'center'},chipNumTxt:{color:'#fff',fontSize:9,fontWeight:'800'},chipName:{fontSize:11,fontWeight:'700'},
  ...
  itemActive:{backgroundColor:'#EEF4FF'},itemLeft:{flexDirection:'row',alignItems:'center',gap:12},
  avatar:{width:36,height:36,borderRadius:18,backgroundColor:'#EEF4FF',justifyContent:'center',alignItems:'center'},
  avatarActive:{backgroundColor:'#1565C0'},avatarTxt:{fontSize:15,fontWeight:'800',color:'#1565C0'},avatarTxtActive:{color:'#fff'},
  itemName:{fontSize:14,fontWeight:'600',color:'#333'},itemNameActive:{color:'#1565C0',fontWeight:'700'},
  mesinIcon:{width:36,height:36,borderRadius:10,backgroundColor:'#EEF4FF',justifyContent:'center',alignItems:'center'},
  emptyBox:{alignItems:'center',paddingVertical:40,gap:10},emptyTxt:{fontSize:13,color:'#bbb'},
});