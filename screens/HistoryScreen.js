...
const cloneShift = (shift) => {
  ...
  return {
    ...
    rows: (shift.rows || []).map((r) => ({
      ...
      rejectMode: r.rejectMode || 'kg',
      estimasiRejectPcs: r.estimasiRejectPcs || 0,
      estimasiRejectKg: r.estimasiRejectKg || 0,
...
// ─── RowEditor ────────────────────────────────────────────────────────────────
function RowEditor({ row, idx, onUpdate, onRemove, canRemove, beratProduk }) {
  ...
  return (
    <View style={edit.rowCard}>
      ...
      <FieldInput
        ...
        onChangeText={(v) => update('permasalahan', v)}
        multiline
      />
      {/* 🔥 SWITCH KG / PCS */}
      <View
      ...
      >
        {['kg', 'pcs'].map((mode) => {
          const currentMode = row.rejectMode || 'kg';
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                const gram = parseFloat(String(beratProduk || '0').replace(',', '.'));

                const value = parseFloat(String(row.totalReject || '0').replace(',', '.'));
                ...
                // 🔥 KG → PCS
                if (currentMode === 'kg' && mode === 'pcs') {
                  const pcs = gram && value ? Math.round((value * 1000) / gram) : 0;

                  newRow = {
                    ...newRow,

                    totalReject: String(pcs),

                    estimasiRejectPcs: pcs,
                ...
                // 🔥 PCS → KG
                  ...
                  const kg = gram && value ? (value * gram) / 1000 : 0;

                  newRow = {
                    ...newRow,

                    totalReject: String(Number(kg).toFixed(2)),
              ...
              style={{
                ...
                backgroundColor: active ? '#1565C0' : '#EEF4FF',
      ...
      <FieldInput
        label={row.rejectMode === 'pcs' ? 'Total Reject (PCS)' : 'Total Reject (KG)'}
        value={row.totalReject}
        onChangeText={(v) => {
          update('totalReject', v);

          const gram = parseFloat(String(beratProduk || '0').replace(',', '.'));

          const value = parseFloat(String(v || '0').replace(',', '.'));

          // 🔥 MODE KG
          if ((row.rejectMode || 'kg') === 'kg') {
            const pcs = gram && value ? Math.round((value * 1000) / gram) : 0;

            update('estimasiRejectPcs', pcs);

            update('estimasiRejectKg', value || 0);
          }
          ...
          // 🔥 MODE PCS
            ...
            update('estimasiRejectKg', kg);

            update('estimasiRejectPcs', value || 0);
          }
        }}
      ...
      >
        {(row.rejectMode || 'kg') === 'kg'
          ? `Estimasi: ${Number(row.estimasiRejectPcs || 0).toLocaleString('id-ID')} PCS`
          : `Estimasi: ${(Number(row.estimasiRejectKg) || 0).toFixed(2)} KG`}
      </Text>
      <FieldInput
        label="Penanganan"
        value={row.penanganan}
        onChangeText={(v) => update('penanganan', v)}
        multiline
      />
      <FieldInput
        label="Nama Asisten"
      ...
      <View style={edit.statusRow}>
        ...
        <View style={edit.statusToggleWrap}>
          ...
          >
            <Text style={[edit.statusBtnTxt, row.status === 'open' && { color: '#c62828' }]}>
              OPEN
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            <Text style={[edit.statusBtnTxt, row.status === 'close' && { color: '#2e7d32' }]}>
              CLOSE
            </Text>
          </TouchableOpacity>
        </View>
...
// ─── ShiftEditor ──────────────────────────────────────────────────────────────
function ShiftEditor({ shiftNum, shift, onUpdate, enabled, onToggle, beratProduk }) {
        ...
        <View style={edit.shiftBody}>
          <View style={edit.shiftFieldRow}>
            <View style={{ flex: 1 }}>
              <FieldInput
                label="Output (pcs)"
                value={shift.output}
                onChangeText={(v) => update('output', v)}
                keyboardType="numeric"
              />
            </View>
            <View style={{ width: 12 }} />
            <View style={{ flex: 1 }}>
              <FieldInput
                label="Cavity"
                value={shift.cavity}
                onChangeText={(v) => update('cavity', v)}
                keyboardType="numeric"
              />
            </View>
          </View>
          <View style={edit.shiftFieldRow}>
            <View style={{ flex: 1 }}>
              <FieldInput
                label="Cycle Time"
                value={shift.cycleTime}
                onChangeText={(v) => update('cycleTime', v)}
                keyboardType="numeric"
              />
            </View>
            <View style={{ width: 12 }} />
...
function EditModal({ item, onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    const calculateRows = (shift) => {
      if (!shift?.rows) return shift;

      return {
        ...shift,

        rows: shift.rows.map((row) => {
          const gram = parseFloat(String(item.berat || '0').replace(',', '.'));

          const total = parseFloat(String(row.totalReject || '0').replace(',', '.'));

          // 🔥 MODE KG
          if ((row.rejectMode || 'kg') === 'kg') {
            const pcs = gram && total ? Math.round((total * 1000) / gram) : 0;

            return {
              ...row,
              estimasiRejectPcs: pcs,
              estimasiRejectKg: total,
            };
          }

          // 🔥 MODE PCS
          else {
            const kg = gram && total ? (total * gram) / 1000 : 0;

            return {
              ...row,
              estimasiRejectKg: kg,
              estimasiRejectPcs: total,
            };
          }
        }),
      };
    ...
    cloned.shift1 = calculateRows(cloned.shift1);
    cloned.shift2 = calculateRows(cloned.shift2);
    cloned.shift3 = calculateRows(cloned.shift3);
  ...
  // ── Fetch master data sesuai bagianProduksi item ──────────────────────
  // bagianProduksi dikunci → tidak berubah selama sesi edit
  const {
    produkList,
    mesinList,
  ...
  // Adaptasi: value = kode (untuk matching dengan form.kodeProduk)
  const produkOptions = useMemo(
    () => produkList.map((p) => ({ value: p.kode, label: p.label, kode: p.kode })),
    [produkList]
  );

  // mesinList sudah { value: noMesin, label: noMesin }
  const mesinOptions = useMemo(() => mesinList, [mesinList]);
  ...
  // ⚠️ Semua hook HARUS dipanggil sebelum early return
  if (!item || !form) return null;

  // Pilih produk → auto-isi namaProduk + kodeProduk sekaligus
  const handleSelectProduk = (kodeVal) => {
    const found = produkOptions.find((p) => p.value === kodeVal);
    if (found) {
      setForm((prev) => ({
        ...prev,
        namaProduk: found.label,
        kodeProduk: found.value,
      }));
    }
  };
  ...
  const toggleShift = (num) => {
    setShiftOn((prev) => {
      ...
      setForm((f) => {
        if (!nowOn) return { ...f, [`shift${num}`]: null };
        if (nowOn && !f[`shift${num}`]) return { ...f, [`shift${num}`]: emptyShift() };
        return f;
      });
  ...
  const cleanShift = (shift, isOn) => {
    if (!isOn) return null;
    if (!shift) return null;
    if (!shift.output && !shift.karu && (!shift.rows || shift.rows.length === 0)) return null;
    return shift;
  };
            ...
            <ScrollView>
              ...
              >
                ...
                <View style={edit.section}>
                  ...
                  {/* Bagian Produksi — READ ONLY (dikunci) */}
                  <View style={edit.fieldWrap}>
                    <Text style={edit.fieldLabel}>Bagian Produksi</Text>
                    <View style={edit.readOnlyBox}>
                      <Ionicons
                        ...
                        style={{ marginRight: 6 }}
                      />
                      <Text style={edit.readOnlyText}>{form.bagianProduksi || '-'}</Text>
                    </View>
                    <Text style={edit.readOnlyHint}>Bagian produksi tidak dapat diubah</Text>
                  </View>

                  {/* Nama Produk — SearchableDropdown dari database */}
                  {masterLoading ? (
                    <View style={edit.fieldWrap}>
                      <Text style={edit.fieldLabel}>Nama Produk</Text>
                      <View style={edit.loadingBox}>
                        <ActivityIndicator size="small" color="#1565C0" />
                        <Text style={edit.loadingBoxTxt}>Memuat data produk...</Text>
                      </View>
                    </View>
                  ) : (
                    <View style={edit.fieldWrap}>
                      <SearchableDropdown
                        ...
                        value={form.kodeProduk} // match by kode
                  ...
                  {/* Kode Produk — auto-filled saat pilih nama produk, read-only */}
                  <View style={edit.fieldWrap}>
                    <Text style={edit.fieldLabel}>Kode Produk</Text>
                    <View style={edit.readOnlyBox}>
                      <Ionicons
                        ...
                        style={{ marginRight: 6 }}
                      />
                      <Text style={edit.readOnlyText}>{form.kodeProduk || '-'}</Text>
                    </View>
                    <Text style={edit.readOnlyHint}>Otomatis terisi saat memilih nama produk</Text>
                  </View>

                  {/* No. Mesin — SearchableDropdown dari database */}
                  {masterLoading ? (
                    <View style={edit.fieldWrap}>
                      <Text style={edit.fieldLabel}>No. Mesin</Text>
                      <View style={edit.loadingBox}>
                        <ActivityIndicator size="small" color="#1565C0" />
                        <Text style={edit.loadingBoxTxt}>Memuat data mesin...</Text>
                      </View>
                    </View>
                  ) : (
                    <View style={edit.fieldWrap}>
                      <SearchableDropdown
                        ...
                        value={form.noMesin}
                        onChange={(v) => setForm((p) => ({ ...p, noMesin: v }))}
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  disabled={loading}
                  onPress={() =>
                    Alert.alert('Simpan Perubahan', 'Yakin ingin menyimpan?', [
        ...
        {shift.rows?.map((row, i) => (
          <View key={i} style={styles.rowDetail}>
            ...
            {[
              ...
              [
                row.rejectMode === 'pcs' ? 'Total Reject (PCS)' : 'Total Reject (KG)',

                row.totalReject,
              ],

              [
                row.rejectMode === 'kg' ? 'Estimasi PCS' : 'Estimasi KG',

                row.rejectMode === 'kg'
                  ? `${Number(row.estimasiRejectPcs || 0).toLocaleString('id-ID')} PCS`
                  : `${(Number(row.estimasiRejectKg) || 0).toFixed(2)} KG`,
              ],
...
const BAGIAN_OPTS = [
  { value: '', label: 'Semua' },
  { value: 'PET PF', label: 'PET PF' },
  { value: 'PET SB', label: 'PET SB' },
...
function FilterModal({ visible, onClose, filters, onApply }) {
  ...
  const handleApply = () => {
    onApply({ bagian: localBagian, dateFrom: localDateFrom, dateTo: localDateTo });
    onClose();
  };
  const handleReset = () => {
          ...
          >
            ...
            <View style={flt.chipsWrap}>
              {BAGIAN_OPTS.map((opt) => (
                <TouchableOpacity
                  key={opt.value}
                  style={[flt.chip, localBagian === opt.value && flt.chipActive]}
                  onPress={() => setLocalBagian(opt.value)}
                >
                  <Text style={[flt.chipTxt, localBagian === opt.value && flt.chipTxtActive]}>
                    {opt.label}
                  </Text>
                </TouchableOpacity>
              ))}
              ...
              {[
                ...
                },
                {
                  ...
                  fn: () => {
                    const to = new Date(),
                      fr = new Date();
                    fr.setDate(to.getDate() - 6);
                    setLocalDateFrom(fmtDate(fr));
                    setLocalDateTo(fmtDate(to));
                  },
                },
                {
                  label: '30 Hari',
                ...
                },
                {
                  ...
                  fn: () => {
                    const now = new Date(),
                      fr = new Date(now.getFullYear(), now.getMonth(), 1);
                    setLocalDateFrom(fmtDate(fr));
                    setLocalDateTo(fmtDate(now));
                  },
                },
              ].map((q) => (
...
export default function HistoryScreen() {
  ...
  const activeFilterCount = [filters.bagian, filters.dateFrom || filters.dateTo].filter(
    Boolean
  ).length;
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter(
        (f) =>
          norm(f.namaProduk).includes(q) ||
          norm(f.kodeProduk).includes(q) ||
          norm(f.noMesin).includes(q)
      );
    }
  ...
  const bagianColor = (bagian) => {
    switch (bagian) {
      ...
      case 'PET SB':
        return { bg: '#E8EAF6', text: '#283593' }; // biru lebih gelap
      case 'INJECT':
  ...
  const renderItem = ({ item }) => {
    const totalRejects = [item.shift1, item.shift2, item.shift3]
      ...
      .reduce((sum, r) => {
        // 🔥 kalau input PCS
        if (r.rejectMode === 'pcs') {
          return sum + parseFloat(r.estimasiRejectKg || 0);
        }
      ...
      <TouchableOpacity style={styles.card} onPress={() => setSelected(item)} activeOpacity={0.85}>
        <View style={styles.cardTop}>
          <View style={styles.cardTopLeft}>
            ...
            <Text style={styles.cardProduk} numberOfLines={2}>
              {item.namaProduk || '-'}
            </Text>
            <Text style={styles.cardMeta}>
              {item.tanggal} · {item.noMesin || '-'}
            </Text>
            {!!item.kodeProduk && (
            ...
            {shiftStatuses.length === 0 ? (
              ...
              shiftStatuses.map(({ num, hasOpen }) => (
                ...
                >
                  ...
                  >
                    <Text
                      style={[styles.shiftNumPillTxt, { color: hasOpen ? '#c62828' : '#2e7d32' }]}
                    >
                      S{num}
                    </Text>
                  </View>
    ...
    <View style={styles.container}>
        ...
        <View style={styles.headerActions}>
          ...
          >
            <Ionicons
              name="filter"
              size={16}
              color={activeFilterCount > 0 ? '#fff' : 'rgba(255,255,255,0.8)'}
            />
      ...
      {activeFilterCount > 0 && (
        <View style={styles.activeFilterBar}>
            ...
            <View style={styles.activeFilterChips}>
              ...
              {(filters.dateFrom || filters.dateTo) && (
                <View style={styles.activeChip}>
                  ...
                  <TouchableOpacity
                    onPress={() => setFilters((p) => ({ ...p, dateFrom: '', dateTo: '' }))}
                    hitSlop={{ top: 6, bottom: 6, left: 4, right: 4 }}
                  >
                    <Ionicons name="close" size={11} color="#1565C0" />
                  </TouchableOpacity>
          ...
          <TouchableOpacity
            onPress={() => setFilters({ bagian: '', dateFrom: '', dateTo: '' })}
        ...
        ListEmptyComponent={
          <View style={styles.center}>
            ...
            <Text style={styles.emptyText}>
              {search || activeFilterCount > 0 ? 'Tidak ada hasil' : 'Belum ada form tersimpan'}
            </Text>
          </View>
        }
      />
